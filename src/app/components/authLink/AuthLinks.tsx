...
const AuthLinks = ({ className }) => {
    ...
    const loggedIn = (
        <div className="">
            ...
            <img 
            className="rounded-full max-h-10 ml-3 cursor-pointer hover:scale-125 translate-y-"
            onClick={() => setOpen(!open)} 
            src={data?.user?.image} 
            alt="Profile picture" />
            <div className={classNames('flex flex-col gap-1 absolute rounded mt-1 p-2 bg-gray-800 text-gray-200 translate-y-1/4 -translate-x-1/4', {hidden: !open})}>
